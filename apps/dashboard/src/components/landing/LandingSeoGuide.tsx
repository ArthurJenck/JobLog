...
const FAQS = [
  {
    question: 'Pourquoi utiliser un tracker de candidatures plutôt qu’un tableur ?',
    answer:
  ...
  {
    question: 'JobLog peut-il aider à relancer un recruteur ?',
    answer:
      "Oui. Vous pouvez programmer des rappels email et notifications web pour relancer au bon moment, avec le contexte de la candidature sous les yeux.",
  },
  {
    question: "L'extension navigateur remplace-t-elle la saisie manuelle ?",
    answer:
      "Elle accélère l'import depuis les sites d'emploi compatibles. Vous pouvez aussi coller une URL ou créer une candidature manuellement quand une offre vient d'un autre canal.",
  },
];
...
export function LandingSeoGuide() {
  ...
  return (
    <section id="faq" ref={ref} className="py-24 px-6 bg-background">
      <div className="mx-auto max-w-6xl grid grid-cols-1 lg:grid-cols-[0.95fr_1.05fr] gap-14">
        <div className="flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-4">
            ...
            <p className="text-muted-foreground leading-relaxed" style={reveal(2)}>
              JobLog aide les chercheurs d'emploi en France à garder une vue claire
              sur leurs candidatures, leurs relances recruteur et les ajustements à
              apporter à chaque CV avant de postuler.
            </p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3" style={reveal(3)}>
            {GUIDES.map(({ icon: Icon, title, description }) => (
              <div key={title} className="rounded-xl border bg-muted/20 p-4">
                <span className="mb-3 inline-flex size-9 items-center justify-center rounded-lg bg-background">
                  <Icon size={17} />
                </span>
        ...
        <div
          className="self-start rounded-2xl border bg-muted/20 px-5 md:px-7"
          style={reveal(4)}
        >
