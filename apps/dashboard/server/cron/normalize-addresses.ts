...
export async function runNormalizeAddresses(limit: number = DEFAULT_BATCH_SIZE) {
...
const normalizeAddressesMethod = method({
  auth: 'cron',
  async handle({ query }) {
    const limit = parseLimit((query as { limit?: string | string[] }).limit);
    return { json: await runNormalizeAddresses(limit) };
  },
});

export default defineHandler({
  GET: normalizeAddressesMethod,
  POST: normalizeAddressesMethod,
});
