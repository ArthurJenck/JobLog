...
export async function runDeleteInactive() {
  const now = new Date();
...
<p>Si vous ne souhaitez pas conserver votre compte, aucune action n'est nécessaire.</p>`,
  ...
  return { warned, deleted, errors: errors.slice(0, 20) };
}

const deleteInactiveMethod = method({
  auth: 'cron',
  async handle() {
    return { json: await runDeleteInactive() };
  },
});

export default defineHandler({
  GET: deleteInactiveMethod,
  POST: deleteInactiveMethod,
});
